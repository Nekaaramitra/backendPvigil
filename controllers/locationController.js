...
const polygons = geoJSON.features.map((feature) => ({
  ...
  wardNo: feature.properties.KGISWardNo, // Add the KGISWardNo to the polygon data
}));
...
const validateLocation = (latitude, longitude) => {
  ...
  for (let i = 0; i < polygons.length; i++) {
    const { coordinates, wardName, wardNo } = polygons[i];
    if (inside([longitude, latitude], coordinates)) {
      // console.log("Inside ward:", wardName);
      return { isInsideCityLimits: true, wardName, wardNo }; // Include wardNo in the response
    }
  }

  // console.log("Not inside any ward.");
  return { isInsideCityLimits: false, wardName: null, wardNo: null }; // Include wardNo in the response
};
